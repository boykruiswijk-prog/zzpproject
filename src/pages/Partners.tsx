...
const partners = [
  ...
  { name: "Circle8", category: "Opdrachten", description: "Via Circle8 actief als zzp'er? ZP Zaken verzorgt de BAV (beroepsaansprakelijkheid) en Cyberverzekering voor zzp'ers die via Circle8 werken.", logo: circle8Logo, icon: Briefcase, link: "/contact", features: ["Opdrachten-marktplaats", "Contractmanagement", "Wet DBA compliant"] },
        ...
        <div className="container-wide">
          <div className="max-w-3xl mx-auto text-center">
            <div className="inline-flex items-center gap-2 bg-primary/10 text-primary px-4 py-2 rounded-full text-sm font-medium mb-6">
              <Users className="h-4 w-4" />{t("partners.becomePartnerBadge")}
            </div>
            <h2 className="mb-6">{t("partners.becomePartnerTitle")}</h2>
            <div className="flex flex-wrap justify-center gap-3 mb-8">
              {(t("partners.becomePartnerTags", { returnObjects: true }) as string[]).map((tag: string) => (
                <span key={tag} className="inline-flex items-center gap-2 bg-card border border-border/50 shadow-sm px-4 py-2 rounded-lg text-sm">
                  <Handshake className="h-4 w-4 text-accent" />{tag}
                </span>
              ))}
            </div>
